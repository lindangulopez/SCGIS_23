...
Path <- "/LiDAR_workshop" ## define your path directory 
...
#lidar_pc <- readLAS(paste0(Path, "/ALS/ALS_Selway.laz"), select = "xyzi", filter = "-keep_first") #  explore options such as filter = "-keep_first", filter = " -drop_z_below 10"
print(lidar_pc)
print(header(lidar_pc))
plot(lidar_pc)
plot(lidar_pc, color = "Z", bg = "white", axis = TRUE, legend = TRUE) # color = ""
plot(lidar_pc, color = "Intensity")
plot(lidar_pc, color = "ScanAngleRank", pal = rainbow)

#writeLAS(lidar_pc, "ALS_Selway.las")


########################### DIGITAL TERRAIN MODEL 

dtm_tin <- rasterize_terrain(lidar_pc, res = 1, algorithm = tin())
#dtm_tin <-  rasterize_terrain(lidar_pc, algorithm = knnidw(k = 6L, p = 2))
...
ground_pc <- filter_ground(lidar_pc)
plot(ground_pc, size = 3)

nlas <- lidar_pc - dtm_tin 
plot(nlas)

nlas2 <- normalize_height(lidar_pc, tin(), dtm = dtm_tin)
lidR::plot(nlas2)
writeLAS(nlas2, paste0(Path, "/outputs/Selway_pcNormalized.laz"))
...
crown_segmentation<- segment_trees(nlas_s, silva2016(chm_s, ttops_5m,  max_cr_factor = 0.4, exclusion = 0.3, ID ="treeID"))
plot(crown_segmentation, bg = "white", size = 4, color = "treeID") # visualize trees
tree10 <- filter_poi(crown_segmentation, treeID == 261)
